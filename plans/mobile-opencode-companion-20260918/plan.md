...
- [in progress] phase-08: user-approved project picker with root/main sessions only; automated work complete and verified 2026-09-21 (plugin 152/152, mobile 30/30, Turbo 6/6; per-file Bastion/Atrium/Crucible [PASS]); remaining: manual Android device check and the user-deferred Lumen ✨ bootstrap (DEBT-003). Implementation authorized by the user 2026-09-20 ("execute the plan"); executing one step at a time under the phase-08 execution rule.
...
- [in progress] phase-11: user-reported Android overflow — the chat screen draws under the status bar and navigation bar and its wrapping picker rows can push the composer off-screen. Approved fix implemented and gated (mobile 32/32, Turbo 6/6; Warden/Atrium/Crucible [PASS]); remaining: manual Android retry to confirm no clipping and a visible composer with the keyboard open.
